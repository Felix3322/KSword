...
- Qt 用户态可用系统默认字体把中英文静态文案预渲染为不透明 BGRA32，再通过独立协议一次上传完整资源集；驱动负责在 `PASSIVE_LEVEL` 生成 24/32 BPP BMP 并调用私有解析器。
- 面板完成 `BgpArm` 后，运行期上传必须通过专用资源更新门进入 `Armed` 解析，且 `BgpBeginDraw` 在取得显示所有权前检查同一状态。上传与崩溃竞争时绘制直接退出，禁止并发调用私有解析器和绘制路径。
- 资源表使用双缓冲：完整解析到非活动集合后原子切换，任何单项失败都销毁暂存集合并保留上一代。崩溃回调只按语言、归因类别和实际 BPP 读取已发布矩形。
- 文字卡片背景必须与崩溃画布一致并保持 alpha=`0xFF`。透明背景在 BGP 32 BPP 路径会产生黑块；源 BMP 继续常驻非分页池，直到该资源集合被安全替换或驱动卸载。
- #174 的 `1280x720` 密集占位面板已停用。当前布局在 2K 屏幕上继续居中使用 `1024x768` 双栏信息层级，页面只保留停止码、人类可读原因、故障模块/偏移/置信度、四个参数和转储操作提示。
