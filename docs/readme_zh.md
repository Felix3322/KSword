...
| 启动项 | 总览、登录、服务、驱动、计划任务、高级注册表、WMI | 启动项分类汇总、图标渲染、过滤/导出、定位文件和注册表位置；可逆启停 Boot/Logon 计划任务并执行任务定义身份校验、结果复核和失败回滚；Run/RunOnce、启动文件夹、机器范围、驱动、服务、关键注册表、Winsock 和 WMI 等来源当前只读保护；不开放永久删除。 |
