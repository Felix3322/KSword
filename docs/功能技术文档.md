...
- 辅助组件包括 `Taskbar/`、`KswordHUD/`、`APIMonitor_x64/`；项目官网独立维护于 [KSwordDEV/Website](https://github.com/KSwordDEV/Website)。
...
- 每类启动项使用对应系统接口枚举。
- 支持过滤、导出、定位文件/注册表、可逆启用/禁用和跳转服务管理；首批可逆条目暂不开放永久删除。
- 右键启停只对后端明确放行的来源开放；界面不根据显示文本猜测注册表、文件或计划任务位置。
...
- 第一阶段可逆操作白名单只包括含 Boot 或 Logon 触发器的计划任务。
- 计划任务动作前重新导出任务 XML，并用忽略 `Enabled` 状态差异的 SHA-256 身份摘要核对枚举快照，防止刷新与点击之间任务定义被替换。
- 计划任务禁用只改变任务启用状态，保留任务定义、触发器和操作；操作完成后独立重新查询状态和身份，失败或超时时再启动一次独立恢复流程，尽量恢复动作前状态。
- HKCU Run/RunOnce、当前用户启动文件夹以及它们已有的恢复记录当前也只读保护：现有路径尚未形成可证明的同事务注册表闭环或基于文件句柄的重解析点安全移动，因此不会用较弱路径冒充可恢复操作。
- HKLM（含 32 位视图）和 ProgramData 启动文件夹统一按机器范围受保护；这样不会把机器级恢复数据降级存入当前用户可写位置。

### 17.4 注意事项

- 服务、驱动、策略项、关键注册表持久化、Winsock、WMI 和未知来源均标记为受保护，禁用启停和永久删除入口。
- 可逆启停前必须展示来源、目标状态、风险原因和恢复方式；同一时间只允许一个启停操作。
- 当前只对通过任务定义身份核验的 Boot/Logon 计划任务开放可逆启停，不开放永久删除；待其他来源具备同等的结构化身份复核和事务恢复 API 后再扩展。
...
- `Taskbar/` 是独立 Qt 工程，随主解决方案构建；启动时会创建 `SosHotkeyLauncher`，在专用线程里监听 `S O S Enter` 固定键盘序列。
...
- Taskbar 的 SOS 功能位于 `Taskbar/SosHotkeyLauncher.cpp` 和 `Taskbar/SosHotkeyLauncher.h`；它通过 `WH_KEYBOARD_LL` 全局低级键盘钩子检测按键按下事件，不注入其它进程。
- SOS 状态机只匹配 `S`、`O`、`S`、`Enter` 四个按键，忽略 Shift/Ctrl/Alt/Win/CapsLock 等修饰键，并丢弃注入键事件；命中后通过 `CreateProcessW` 启动 `Ksword5.1.exe`。
- 主程序路径优先按发行包同目录解析，同时兼容源码树下 `Taskbar/x64/Release` 到 `Ksword5.1/Ksword5.1/x64/Release/Ksword5.1.exe` 的构建输出布局。
