    ...
    )
{
    ...
    // 中文说明：私有表地址不再直接取 HalPrivateDispatchTable 导出。
    // 每个受支持 Build 都从 HalTranslateBusAddress 的小窗口内，以经过
    // byte/mask 描述的 RIP-relative 槽引用反推出表基址；候选必须唯一，
    // 且完整结构范围和 Version/ByteSize 同时通过才会继续枚举。
